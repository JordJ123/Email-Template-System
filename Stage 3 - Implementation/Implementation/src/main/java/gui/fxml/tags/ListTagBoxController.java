 ...
 * @author Jordan Jones
 */
public class ListTagBoxController extends FXMLController {
    ...
    private void addTagValueTextField(String tagValue) throws IOException {
        FXMLComponent<ListTagBoxTextFieldController> tagBoxLoader
            = new FXMLComponent<>("tags/ListTagBoxTextField");
        HBox hBox = (HBox) tagBoxLoader.load();
        if (getListTagValueTextFieldsBox().getChildren().size() == 1) {
            ((HBox) (getListTagValueTextFieldsBox().getChildren().get(0)))
        ...
        tagBoxLoader.getController().setController(this, tagValue);
    }
